...
1. 將多份掃描圖像檔批次製成多份PDF檔。
1. 用 Google Docs 打開多份PDF檔，同時執行 OCR。
1. 快速轉換英文標點符號成繁體中文全型標點符號。
1. 快速轉換英文字母（含大小寫）及阿拉伯數字成全型符號，以利直式排版。
1. （待續）收集準備額外錯別字更新檔。

此流程的優點在於：

1. 讓製書過程流暢化，節省時間。例如，以一本三百頁的中文書而言，若用 Google Doc 逐一打開單頁掃描圖像檔進行 OCR，大約要花四個小時，而且容易遺漏。此流程第一、二步驟總共僅耗費約十五分鐘，且不會遺漏。
1. 第三、四步驟自動將標點符號、英文字母、阿拉伯數字改成全型，減少手改編輯時間。同時將英文字間的空格改成全型，不會在 mPDB 中清除空白字元時被錯誤去除。
1. 使用PDF檔校對，比使用單一掃描影像檔方便。
...
下面提供一個 Python 程式讓你在剪貼板內容中搜索和替換多個標點符號。你可以通過創建桌面捷徑的方式來執行此程式。另外，文字中若有英文或阿拉伯數字，也會被轉成全型（例，"A" 轉成"Ａ"; "1" 轉成"１"），以方便製作直式 updb 或 epub 檔。
